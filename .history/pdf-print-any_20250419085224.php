...
class PDF_Print {
    ...
    public function handle_pdf_generation() {
        ...
        if ($is_agenda_event) {
            ...
            error_log('PDF Print: Agenda sections count: ' . (is_array($agenda_sections) ? count($agenda_sections) : 'Not an array'));
            ...
            // Fallback to regular content if agenda content is too short
            if (strlen($html_content) < 100) {
                error_log('PDF Print: Agenda content too short, falling back to regular content');
                ob_start();
                the_content();
                $fallback_content = ob_get_clean();
                error_log('PDF Print: Fallback content length: ' . strlen($fallback_content));
                
                if (strlen($fallback_content) > strlen($html_content)) {
        ...
        if (strlen($html_content) < 50) {
            error_log('PDF Print: Content too short to generate PDF: "' . $html_content . '"');

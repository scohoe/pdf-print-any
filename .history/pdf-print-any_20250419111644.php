    ...
    public function handle_pdf_generation() {
        ...
        if ($is_agenda_event) {
            ...
            if (function_exists('display_agenda_items_shortcode')) {
                ...
                error_log('PDF Print: Agenda shortcode content length: ' . strlen($html_content));
                
                // Check if agenda shortcode returned minimal content
                if (strlen($html_content) < 50 || strpos($html_content, 'No agenda items available') !== false) {
                    // Try to get regular content as fallback
                    ob_start();
                    the_content();
                    $fallback_content = ob_get_clean();
                    error_log('PDF Print: Using fallback content, length: ' . strlen($fallback_content));
                    
                    if (strlen($fallback_content) >= 50) {
